...
import { getCharacterCards } from "@/lib/lostark/client";
...
import type { CharacterCardsResponse } from "@/types/character";
...
export async function GET(
  ...
  const rateLimit = checkRateLimit(
    `character-cards-route:${clientIp}`,
  ...
  try {
    const cacheKey = `character-cards:${characterName}`;
    const { value, cacheHit } = await withCache(
      cacheKey,
      CACHE_TTL_MS.CHARACTER_ARMORY,
      () => getCharacterCards(characterName)
    );

    if (value.armoryCard === null) {
      return NextResponse.json(
    ...
    const responseBody: CharacterCardsResponse = {
      armoryCard: value.armoryCard,
      dataTimestamp: new Date().toISOString(),
      cacheHit,
      sources: [
        { field: "cards", origin: "API", fetchedAt: new Date().toISOString() },
      ],
    };
