...
import { getCharacterCombatSkills } from "@/lib/lostark/client";
...
import type { CharacterCombatSkillsResponse } from "@/types/character";
...
export async function GET(
  ...
  const rateLimit = checkRateLimit(
    `character-combat-skills-route:${clientIp}`,
  ...
  try {
    const cacheKey = `character-combat-skills:${characterName}`;
    const { value, cacheHit } = await withCache(
      cacheKey,
      CACHE_TTL_MS.CHARACTER_ARMORY,
      () => getCharacterCombatSkills(characterName)
    );
    ...
    const responseBody: CharacterCombatSkillsResponse = {
      skills: value.skills,
      dataTimestamp: new Date().toISOString(),
      cacheHit,
      sources: [
        { field: "combat-skills", origin: "API", fetchedAt: new Date().toISOString() },
      ],
    };
