...
import { getCharacterEquipment } from "@/lib/lostark/client";
...
import type { CharacterEquipmentResponse } from "@/types/character";
...
export async function GET(
  ...
  const rateLimit = checkRateLimit(
    `character-equipment-route:${clientIp}`,
  ...
  try {
    const cacheKey = `character-equipment:${characterName}`;
    ...
    const responseBody: CharacterEquipmentResponse = {
      equipment: value.equipment,
      dataTimestamp: new Date().toISOString(),
      cacheHit,
      sources: [
        { field: "equipment", origin: "API", fetchedAt: new Date().toISOString() },
      ],
    };
